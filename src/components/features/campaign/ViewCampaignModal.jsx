...
export default function ViewCampaignModal({ campaign: c, onClose, onEdit }) {
  if (!c) return null;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      ...
      >
        ...
        >
          <div>
            <div className="text-xs font-display font-semibold uppercase tracking-widest mb-1" style={{ color: "var(--color-primary)" }}>
              Campaign Details
            </div>
          ...
          <button onClick={onClose} className="p-1.5 rounded-lg transition-all hover:opacity-70 mt-0.5" style={{ color: "var(--color-text-muted)" }}>
            <X size={17} />
          </button>
        </div>
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <InfoCard icon={<MapPin size={14} />} label="State" value={c.state} />
            <InfoCard icon={<LayoutGrid size={14} />} label="Walls" value={c.walls} />
            <InfoCard icon={<Calendar size={14} />} label="Start Date" value={c.startDate} mono />
            <InfoCard icon={<Calendar size={14} />} label="End Date" value={c.endDate} mono />
          </div>
          ...
          >
            ...
            {(c.assignedUsers || []).length > 0 ? (
              ...
              <span className="text-xs" style={{ color: "var(--color-text-muted)" }}>No users assigned</span>
            )}
          </div>
        </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5 mb-1">
        ...
        <span className="text-xs font-display font-semibold uppercase tracking-widest" style={{ color: "var(--color-text-muted)" }}>
          {label}
        </span>
      </div>
      <div
